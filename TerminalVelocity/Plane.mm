...
#define WAIT_TIME WAIT_TIME_PLANE
...
-(id)initRandomInWorld:(b2World *)world {
    ...
    return [self initWithWorld:world atLocation:ccp(randomNum, -50)]; 
}

-(id)initWithWorld:(b2World *)world atLocation:(CGPoint)location {
    self = [self initWithSpriteFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"plane00001.png"]];
    if (self) {         
...
-(void)resetObject {
    if(maxPlanes <= MAX_SECONDARY_OBJECT)
    {
       // PLAYSOUNDEFFECT(PLANE);
        ...
        waitTime = arc4random_uniform(WAIT_TIME_PLANE) + 3;
        ...
        b2Vec2 pos;
        pos.Set(randomNum/PTM_RATIO, -50/PTM_RATIO);
        body->SetTransform(pos, 0);
        self.position = ccp(randomNum, -50);
        self.visible = NO;
...
-(void)updateStateWithDeltaTime:(ccTime)deltaTime andListOfGameObjects:(CCArray*)listOfGameObjects {
    ...
    if (characterState == kStateIdle) {
        ...
        GKAchievement* achievement = [gkHelper getAchievementByID:@"skyWriter"];//skyWriter haha
        ...
        [gkHelper reportAchievementWithID:achievement.identifier percentComplete:percent];        
         ...
         [self objectsAchievmentIsComplete:achievement.identifier achievementTitle:@"Sky Writer"];
