...
-(id)initWithWorld:(b2World *)world atLocation:(CGPoint)location {
    self = [self initWithSpriteFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"pigeon00001.png"]];
    if (self) {
...
-(id)init {
    ...
    if (self) {
        ...
        [self setFlyingAnim:[self loadPlistForAnimationWithName:@"pigeonFlyingAnim" andClassName:@"GameObjects"]];
    }
    return self;
}
...
-(void)updateStateWithDeltaTime:(ccTime)deltaTime andListOfGameObjects:(CCArray*)listOfGameObjects {
    ...
    if (characterState == kStateIdle) {
        ...
        GKAchievement* achievement = [gkHelper getAchievementByID:@"pestControl"];//pestControl
        ...
        [self objectsAchievmentIsComplete:achievement.identifier achievementTitle:@"Pest Control"];
